...
import json
try:
    import torch.utils.data as tdata  # type: ignore
except Exception:
    tdata = None
import argparse
...
def get_detected_keypoints(image_id: str) -> Tuple[list, np.ndarray]:
...
def detect_keypoints(image_file: Union[str, os.PathLike]):
    """
...
def create_feature_matches(image_file1: Union[str, os.PathLike], image_file2: Union[str, os.PathLike],
                           lowe_ratio: float = 0.6, min_matches: int = 10):
    """
...
def create_ransac_matches(image_file1: Union[str, os.PathLike], image_file2: Union[str, os.PathLike],
    ...
    if essential_mtx is None or mask is None:
        return match_id
    if essential_mtx.shape != (3, 3):
        essential_mtx = essential_mtx[:3, :3]
    is_inlier = mask.astype(bool)
...
def create_scene_graph(image_files: list, min_num_inliers: int = 40):
    ...
    for i in range(len(image_ids)):
        for j in range(i + 1, len(image_ids)):
            match_file = os.path.join(RANSAC_MATCH_DIR, f"{image_ids[i]}_{image_ids[j]}.npy")
