...
def get_init_image_ids(scene_graph: dict) -> (str, str):
    ...
    for id1, nbrs in scene_graph.items():
        for id2 in nbrs:
            a, b = sorted([id1, id2])
            if (a, b) in seen:
                continue
            ...
            match_file = os.path.join(RANSAC_MATCH_DIR, f"{a}_{b}.npy")
            if not os.path.exists(match_file):
                continue
            num_inliers = np.load(match_file).shape[0]

            cand = (num_inliers, a, b)
            if cand > best:
                best = cand
...
def visualize_point_cloud(pts: np.ndarray):
    if o3d is None:
        raise ImportError('open3d is not installed; cannot visualize point cloud')
...
def get_init_extrinsics(image_id1: str, image_id2: str, intrinsics: np.ndarray) -> (np.ndarray, np.ndarray):
    ...
    """ YOUR CODE HERE """
    _, rotation_mtx, tvec, _ = cv2.recoverPose(
        essential_mtx,
        points2d_1.astype(float),
        points2d_2.astype(float),
        intrinsics,
    )
    extrinsics2[:3, :3] = rotation_mtx
    extrinsics2[:, 3] = tvec.reshape(3)
...
def get_reprojection_residuals(points2d: np.ndarray, points3d: np.ndarray, intrinsics: np.ndarray,
    ...
    """ YOUR CODE HERE """
    points3d_cam = (rotation_mtx @ points3d.T).T + tvec.reshape(1, 3)
    proj = (intrinsics @ points3d_cam.T).T
    proj = proj[:, :2] / proj[:, 2:3]
    residuals = np.sqrt(np.sum((proj - points2d) ** 2, axis=1))
...
def solve_pnp(image_id: str, point2d_idxs: np.ndarray, all_points3d: np.ndarray, point3d_idxs: np.ndarray,
    ...
    for _ in range(num_ransac_iterations):
        ...
        ok, rvec, tvec = cv2.solvePnP(
            objectPoints=selected_pts3d.astype(float),
            imagePoints=selected_pts2d.astype(float),
            cameraMatrix=intrinsics.astype(float),
            distCoeffs=None,
...
def get_next_pair(scene_graph: dict, registered_ids: list):
    ...
    for registered_id in registered_ids:
        for new_id in scene_graph[registered_id]:
            if new_id in registered_set:
                continue
            num_inliers = load_matches(new_id, registered_id).shape[0]
            if num_inliers > max_num_inliers:
                max_num_inliers = num_inliers
                max_new_id = new_id
